...
こうして得た「中心から近い順の六角格子点」を円軌道の基準式（節 5.3, 7 参照）で変換すれば、見た目も円盤状に広がる編隊初期状態が得られる。

---

## 6. 正方格子ディスク配置の拡張

六角格子は最小距離が6方向に揃う利点があるが、運用シナリオによっては「最短距離の衛星が4機になる」ような正方格子（square lattice）配置が望まれる。ここでは整数格子 $\mathbb{Z}^2$ を用いたディスク切り出し手法を整理する。

### 6.1 整数格子と直交座標

* 各衛星候補は整数対 $(i, j)$ で表す。
* 直交座標はそのまま $(x, y) = (i, j)$、最近傍間隔は1とする。

### 6.2 極座標とディスク切り出し

$$
\rho = \sqrt{i^2 + j^2}, \qquad \theta = \text{atan2}(j, i)
$$

* 4近傍（上下左右）だけを使った幅優先探索ではマンハッタン距離順になってしまうため、**優先度付きキュー（min-heap）**で $\rho$ が小さい格子点から順番に展開する。
* 処理手順: (1) $(0,0)$ をキューに追加、(2) $\rho$ が最小の点を取り出して採用、(3) その4近傍を極座標化して未訪問ならキューへ追加。
* これを $X$ 個採用できるまで繰り返すと、ユークリッド距離が厳密に昇順になるため、外縁もほぼ円形を維持しつつ最近傍が常に4方向の正方格子点で構成される。

### 6.3 スケーリングと配置モード

* **radius モード**: 採用点の最大距離 $\rho_{\max}$ を求め、所望の半径 $R$ に合わせて $\rho \mapsto \rho (R/\rho_{\max})$。
* **spacing モード**: ユーザー指定の間隔 $S$ を直接掛ける ($\rho \mapsto \rho S$)。正方格子の最短距離は1なので、$S$ がそのまま隣接4機との距離になる。

擬似コードは以下の通り。

```pseudocode
function disk_points_from_square(X):
    if X <= 0: return []
    heap = PriorityQueue(order by rho then theta)
    visited = set()
    push(heap, (0,0, rho=0, theta=0))
    visited.add((0,0))
    points = []
    while |points| < X:
        p = pop_min(heap)
        ...
        for (dx, dy) in [(1,0),(-1,0),(0,1),(0,-1)]:
            ...
            if q not in visited:
                visited.add(q)
                push(heap, (q.x, q.y, rho(q), theta(q)))
    return points
```

この手順で得た極座標を節 5.3 と同じ線形変換に通せば、HCW ベースの初期位置・速度を即座に生成できる。
